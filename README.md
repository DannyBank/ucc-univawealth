...
- **Java 21** + **JavaFX 21** (Controls, FXML, Swing interop)
- **SQLite** (via `sqlite-jdbc`) for local, file-based persistence — no external database server needed
...
The architecture roughly follows MVC: FXML defines the view, controllers handle UI logic, repositories handle persistence, and services (auth, session, app-wide data caching) sit in between.
...
(Or open the `.db` file in any SQLite client — DB Browser for SQLite works well — and run the three scripts in order: tables, then seed data, then indexes.)
...
- **`AppData` acts as a lightweight in-memory cache** of the logged-in user's savings, investments, and transactions, refreshed from the database when the dashboard needs current numbers — so we're not hitting SQLite on every UI redraw.
...
This started as a coursework project, so a few things are still rough edges:
...
- Test coverage (JUnit 5 is already a dependency) is minimal — models and repositories would benefit from unit tests.
...
A nod to the university context this was built in — a "universal wealth" tracker, scoped down to something one student could actually build and test in a semester.

---

*Built as part of an MSc. Information Technology program — feedback and pull requests welcome if you're extending it further.*
